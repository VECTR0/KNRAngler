    ...
    public partial class CloudForm : Form
    {
        ...
        private void CloudForm_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    double de = j * Math.PI * 2 / n;
                    double ro = i * Math.PI * 2 / n;
                    float x = .9f + (float)( Math.Cos(de) * Math.Sin(ro));
                    float y = (float)( Math.Sin(de) * Math.Sin(ro));
                    float z = .9f +(float)(  Math.Cos(ro));
                    if(z < 0)AddPoint(x, y, z, 200, 250, 250);
                    else AddPoint(x, y, z, 200/2, 250/2, 250/2);
                }
            }
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    double de = j * Math.PI * 2 / n;
                    double ro = i * Math.PI * 2 / n;
                    float x = (float)( Math.Cos(de) * Math.Sin(ro)) * .99f;
                    float y = 1+(float)( Math.Sin(de) * Math.Sin(ro)) * .99f;
                    float z = (float)(  Math.Cos(ro)) * .99f;
                    if(z<0)AddPoint(x, y, z, 150, 150, 100);
                    else AddPoint(x, y, z, 150 / 2, 150 / 2, 100 / 2);
                }
            }
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    double de = j * Math.PI * 2 / n;
                    double ro = i * Math.PI * 2 / n;
                    float x = .9f + (float)( Math.Cos(de) * Math.Sin(ro))*.99f;
                    float y = (float)( Math.Sin(de) * Math.Sin(ro)) * .99f;
                    float z = .9f +(float)(  Math.Cos(ro)) * .99f;
                    if(z < 0)AddPoint(x, y, z, 100, 150, 150);
                    else AddPoint(x, y, z, 100/2, 150/2, 150/2);
                }
            }*/
        ...
        private void tmrRender_Tick(object sender, EventArgs e)
        {
            ...
            unsafe
            {
                ...
                {
                    float[] p = points[i];                                                                                  
                    float[] pt = pointsTrans[i];
                    //float[] ps = pointsScreen[i];
                    
                    pt[0] = p[0] * c + p[2] * s;
                    pt[1] = p[0] * s * S + p[1] * C - p[2] * c * S;
                    pt[2] = trbTranslateZValue - (-p[0] * s * C + p[1] * S + p[2] * c * C);
                    //pt[3] = 1;
                    // Vector<double> vf = Vector<double>.Build.DenseOfArray(new double[] { vp[0], vp[1], vp[2], 1 });
                    //vf = mp * vf;
